...
func ProcessContent(ctx context.Context, source string, content []byte, opts Options, embedder llm.Embedder, store vector.Store) (int, error) {
	...
	// Examples: "/docs/course/lesson1.md" -> "lesson1.md", "notes/todo.txt" -> "todo.txt".
	...
	// Remove any chunks previously ingested for this source before indexing the
	// new content. For example, if "employees.csv" is updated with new phone
	// numbers, deleting its old chunks first ensures only the latest version is
	// stored and prevents stale/orphaned chunks from remaining in the vector store.
	//
	// If this is a new file, there's nothing to delete, and the call is
	// inexpensive.
	...
	for i, c := range chunks {
		docs[i] = vector.Document{
			...
			Metadata: map[string]string{
				"source":      base, // Which file this chunk came from, original source filename
				"chunk_index": strconv.Itoa(i),
				"chunks":      strconv.Itoa(len(chunks)), // Total number of chunks for this document
